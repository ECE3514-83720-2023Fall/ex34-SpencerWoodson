...
void BinaryTree<TreeItemType, FunctionType>::copyTree(
  ...
  if (treePtr != nullptr) { 
    //TODO
    // copy root node
    newTreePtr = new NodeType(treePtr->item, nullptr, nullptr);
    // copy left subtree
...
void BinaryTree<TreeItemType, FunctionType>::preorder(NodeType *treePtr,
  ...
  if (treePtr != nullptr) {
   //TODO
   TreeItemType item = treePtr->item;
   visit(treePtr->item);            // Visit the root
   preorder(treePtr->leftChildPtr, visit);   // Traverse the left subtree
...
void BinaryTree<TreeItemType, FunctionType>::inorder(NodeType *treePtr,
  ...
  if (treePtr != nullptr) {
   //TODO
   inorder(treePtr->leftChildPtr, visit);    // Traverse the left subtree
   visit(treePtr->item);                     // Visit the root
   inorder(treePtr->rightChildPtr, visit);   // Traverse the right subtree
...
void BinaryTree<TreeItemType, FunctionType>::postorder(NodeType *treePtr,
  ...
  if (treePtr != nullptr) {
   //TODO
   postorder(treePtr->leftChildPtr, visit);   // Traverse the left subtree
   postorder(treePtr->rightChildPtr, visit);  // Traverse the right subtree
   visit(treePtr->item);                      // Visit the root
...
BinaryTree<TreeItemType, FunctionType> BinaryTree<TreeItemType, FunctionType>::makeBinaryTree(const TreeItemType *array, int low, int high) {
    ...
    //TODO
    /*if (low > high) {
        return BinaryTree<TreeItemType, FunctionType>();  // Return an empty tree
    }

    int mid = (low + high) / 2;
    BinaryTree<TreeItemType, FunctionType> resultTree(array[mid]);

    // Recursively construct the left and right subtrees
    resultTree.attachLeftSubtree(makeBinaryTree(array, low, mid - 1));
    resultTree.attachRightSubtree(makeBinaryTree(array, mid + 1, high));

    return resultTree;*/
    std::cout << "low, high = " << low << " " << high << std::endl;
    //TODO
    NodeType* newTreePtr = nullptr;
    return newTreePtr;
}
